...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item $\Sigma$ is an $m \times n$ "diagonal" matrix with positive real entries on the main diagonal in decreasing order (these non-zero diagonal entries are called the \textbf{singular values}), and
    ...
    \begin{enumerate}
        ...
        \item Square root the $r$ non-zero values of $D$ to find the $m\times n$ matrix $\Sigma$. If any eigenvalue is negative, simply apply the negative to the respective eigenvector, but only in either $U$ or $V$ but not both.
        \item Letting $r = \text{rank}(A)$ (or equivalently the number of singular values), use
        \begin{align*}
